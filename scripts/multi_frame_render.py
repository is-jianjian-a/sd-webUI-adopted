...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        with gr.Row():
            input_dir = gr.Textbox(label='Input directory', lines=1, value="/home/omnisky/Public/dengzhijian/webUI/stable-diffusion-webui/input")
            output_dir = gr.Textbox(label='Output directory', lines=1, value="/home/omnisky/Public/dengzhijian/webUI/stable-diffusion-webui/outputs/mfvr")
    ...
    def run(self, p, append_interrogation, input_dir, output_dir, first_denoise, third_frame_image, color_correction_enabled, unfreeze_seed, loopback_source, use_csv, table_content, use_txt, txt_path):
        ...
        reference_imgs = [os.path.join(input_dir, f) for f in sorted(os.listdir(input_dir)) if re.match(r'.+\.(jpg|png)$', f)]
        ...
        for i in range(loops):
            ...
            info = processing.create_infotext(p, p.all_prompts, p.all_seeds, p.all_subseeds, comments, 0, 0)
            pnginfo = {}
            if info is not None:
            ...
            if extension.lower() == '.png':
                ...
                init_img.save(os.path.join(output_dir, filename), pnginfo=pnginfo_data)

            elif extension.lower() in ('.jpg', '.jpeg', '.webp'):
                init_img.save(os.path.join(output_dir, filename))

                if opts.enable_pnginfo and info is not None:
                    piexif.insert(exif_bytes(), os.path.join(output_dir, filename))
            else:
                init_img.save(os.path.join(output_dir, filename))
